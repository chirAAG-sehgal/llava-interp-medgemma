...
def create_interactive_logit_lens(hidden_states, norm, lm_head, processor, image, model_name, image_filename, input_ids,message,  save_folder = ".", image_size=896, patch_size=56, misc_text=""):
    # Tokenize the prompt
    # input_ids = tokenizer.encode(prompt)
    prompt = message
    # Find the image token and replace it with image tokens
    img_token_id = processor.image_token_id  # The token ID for <img>
    # img_token_count = (896 // 14) ** 2  # 576 for 336x336 image with 14x14 patches, 256 for our case in SIGLIP
    ...
    for token_id in input_ids:
            if token_id.item() == img_token_id:
                ...
                token_labels.append(processor.tokenizer.decode([token_id.item()]))
    ...
    for layer in range(num_layers):
        ...
        for pos in range(sequence_length):
            top_5_tokens = [processor.tokenizer.decode(idx.item()) for idx in top_5_indices[0, pos]]
